...
**Trigger**: user drops a file into `wiki/raw/` and says "ingest this", OR pastes content and says "ingest this into the wiki", OR asks to scan optional external inboxes such as Obsidian `Clippings`.

**Helper skill**: if your agent/runtime supports repo-local skills, use `skills/wiki-ingest/SKILL.md` as the operational guide for ingest.

**Steps**:

1. **Archive raw**. Treat `wiki/raw/` as the default immutable inbox. Files may live directly under `wiki/raw/` without category folders. If the user already placed a file there, leave it in place. If the source comes from pasted text or an external inbox, copy it into `wiki/raw/` first. Once in `raw/`, **never modify**.

2. **Optional external scan**. If the user opts in, scan for Obsidian `Clippings` with `python skills/wiki-ingest/scripts/scan_pending_sources.py --include-obsidian-clippings`. If Obsidian is not installed or no `Clippings` folder is found, skip this source silently and continue with `wiki/raw/`.

3. **Compile to source-summary**. Create `wiki/sources/<YYYY-MM-DD>-<slug>.md` with this structure:
   ```markdown
   ---
...
4. **Extract entities & concepts**. Identify every entity and concept the source mentions. For each:
...
5. **Append to log**. Add a row to `wiki/_log.md`:
   ```
   | YYYY-MM-DD HH:MM | ingest | raw/<file> | sources/<file> + N updates | <one-line note> |
   ```

6. **Update inbox digest**. Add a row to the current week's table in `wiki/inbox-digest.md`:
   ```
...
7. **Report**. Tell the user: "Ingested into `sources/<file>`. Updated `<entity1>`, `<concept1>`. Created N cross-references. Flagged 0 contradictions." (If contradictions, see Protocol 3.)
