...
├── raw/                # immutable original materials; flat inbox by default
...
sources: [raw/filename]            # only for source-summary
...
1. Move or copy the original to `raw/` (immutable archive; category subfolders are optional, not required)
...
## Optional external inboxes

- Obsidian `Clippings` can be treated as an opt-in external inbox.
- Auto-discovery is available via `python skills/wiki-ingest/scripts/scan_pending_sources.py --include-obsidian-clippings`.
- If Obsidian is not installed or no `Clippings` folder exists, ignore that source and continue.
